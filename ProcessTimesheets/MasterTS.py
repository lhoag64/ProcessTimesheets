...
class Master:
  ...
  def Init(root):
    ...
    Master.read(os.path.join(root,masterSS))
  ...
  def read(fname):
    ...
    TimeSheet.TSCode.Init(codes)
    TimeSheet.TSLocation.Init(locations)
    TimeSheet.TSActivity.Init(activities)
    TimeSheet.TSProduct.Init(products)
  ...
  def readCol(ws,col):
    ...
    #---------------------------------------------------------------------
    # Loop looking for codes and saving them
    #---------------------------------------------------------------------
    wsRow = startRow
    wsCol =  col
    while True:
      code = ws.cell(row = wsRow,column=wsCol).value
      if (code != None):
        if (found == False):
          found = True
          startRow = wsRow
